...
use super::{iri::parse_func_iri_prv, number::parse_digits, sep::skip_ws, FromSvg, ParseError};
...
struct RecgonizedKeyword<'a>(&'a str);

#[allow(unused)]
impl<'a> Deserializer for &'a mut RecgonizedKeyword<'a> {
...
#[allow(unused)]
impl<'a> NodeAccess for &'a mut RecgonizedKeyword<'a> {
    type Error = mlang_rs::rt::serde::de::Error;
...
fn parse_recognized_keyword(ctx: &mut ParseContext<'_>) -> parserc::Result<Color> {
    ...
    let mut deser = RecgonizedKeyword(value);
